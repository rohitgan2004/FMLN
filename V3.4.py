...
class ExtendedKalmanFilter:
    def __init__(self, num_tickers, initial_states, Q=1e-3, R=0.1, alpha=0.01, mu=100.0):
        ...
        self.alpha = alpha
        self.mu = mu  # scalar mean for simplicity, can be extended to a vector

    def f(self, x):
        # Mean reverting function:
        # x_{t+1} = (1 - alpha)*x_t + alpha*mu
        # Note: the process noise is handled separately by the covariance update
        ...
        for data in responses:
            if data and 'tickers' in data:
                for ticker_data in data['tickers']:
                    ...
                    all_prices[ticker] = {'price': price, 'bid': bid_price, 'ask': ask_price}
            else:
...
def process_ticker(data, positions, current_price, threshold, future_prices, skipped_tickers):
    ...
    # Check future price (10 minutes later)
    ...
    if future_price > price and position == 0:
        ...
        if quantity > 0:
            ...
            # Store the time when bought to sell after 10 minutes
    ...
    # No immediate else action required
...
initial_prices = [prices.get(t, 0.0) for t in tickers]

ekf = ExtendedKalmanFilter(num_tickers=num_tickers, initial_states=initial_prices)

positions = {ticker: 0 for ticker in tickers}
current_price = {ticker: 0.0 for ticker in tickers}
time_interval = 5
threshold = 0.005  # Chosen threshold

buy_time = {}  # Track when we bought a ticker to sell after 10 minutes
    ...
    while True:
        ...
        if start_time <= now_est <= end_time:
            ...
            if len(bid_ask_data) > 0:
                ranked_tickers, skipped_tickers = update_and_rank_tickers(bid_ask_data, max_variation_threshold=0.002*price)        
            else:
                ranked_tickers = []
                skipped_tickers = []
            # EKF steps
            ekf.predict()
            z = [current_price.get(t, 0.0) for t in tickers]
            ekf.update(z)
            estimated_prices = ekf.get_states()

            # Predict future (10 minutes later)
            future_prices = ekf.predict_future(steps=120)  # 120 steps for 10 minutes if each step is 5s
            ...
            # After hours, wait until next day's start or if before market open, wait until open
            if now_est > end_time:
